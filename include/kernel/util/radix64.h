...
public:
    expand3_4Kernel(LLVMTypeSystemInterface & ts, StreamSet * input, StreamSet * expandedOutput);
private:
...
public:
    radix64Kernel(LLVMTypeSystemInterface & ts, StreamSet * input, StreamSet * output);
private:
...
public:
    base64Kernel(LLVMTypeSystemInterface & ts, StreamSet * input, StreamSet * output);
private:
    virtual void generateDoBlockMethod(KernelBuilder & b) override;
    virtual void generateFinalBlockMethod(KernelBuilder & b, llvm::Value * remainingBytes) override;
    llvm::Value* processPackData(KernelBuilder & b, llvm::Value* packData) const;
