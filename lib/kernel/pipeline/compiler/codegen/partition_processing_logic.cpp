...
void PipelineCompiler::branchToInitialPartition(BuilderRef b) {
    ...
    #ifdef ENABLE_PAPI
    readPAPIMeasurement(b, PipelineInput, PAPIReadKernelStartMeasurementArray);
    #endif
    ...
    if (isMultithreaded()) {
        ...
        #ifdef ENABLE_PAPI
        accumPAPIMeasurementWithoutReset(b, PAPIReadKernelStartMeasurementArray, FirstKernel, PAPIKernelCounter::PAPI_KERNEL_SYNCHRONIZATION);
        #endif
    }
 ...
 ** ------------------------------------------------------------------------------------------------------------- */
void PipelineCompiler::phiOutPartitionStatusFlags(BuilderRef b, const unsigned targetPartitionId, const bool fromKernelEntryBlock) {
 ...
 ** ------------------------------------------------------------------------------------------------------------- */
void PipelineCompiler::phiOutPartitionStateAndReleaseSynchronizationLocks(BuilderRef b,
 ...
 ** ------------------------------------------------------------------------------------------------------------- */
void PipelineCompiler::acquirePartitionSynchronizationLock(BuilderRef b, const unsigned firstKernelInTargetPartition, Value * const segNo) {
...
void PipelineCompiler::writeInitiallyTerminatedPartitionExit(BuilderRef b) {
    ...
    if (LLVM_LIKELY((nextPartitionId != jumpTargetId) && mIsPartitionRoot)) {
        ...
        #ifdef ENABLE_PAPI
        accumPAPIMeasurementWithoutReset(b, PAPIReadKernelStartMeasurementArray, mKernelId, PAPIKernelCounter::PAPI_KERNEL_TOTAL);
        #endif
...
void PipelineCompiler::checkForPartitionExit(BuilderRef b) {
    ...
    if (LLVM_LIKELY(nextKernel < PipelineOutput)) {
        #ifdef ENABLE_PAPI
        readPAPIMeasurement(b, nextKernel, PAPIReadKernelStartMeasurementArray);
        #endif
        startCycleCounter(b, {CycleCounter::KERNEL_SYNCHRONIZATION, CycleCounter::TOTAL_TIME});
        ...
        #ifdef ENABLE_PAPI
        accumPAPIMeasurementWithoutReset(b, PAPIReadKernelStartMeasurementArray, nextKernel, PAPIKernelCounter::PAPI_KERNEL_SYNCHRONIZATION);
        #endif
    }
