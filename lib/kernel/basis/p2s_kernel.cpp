...
void p2s_step(KernelBuilder & b, Value * p0, Value * p1, Value * hi_mask, unsigned shift, Value * &s1, Value * &s0) {
    Value * t0 = b.simd_if(1, hi_mask, p0, b.simd_srli(16, p1, shift));
    Value * t1 = b.simd_if(1, hi_mask, b.simd_slli(16, p0, shift), p1);
    s1 = b.esimd_mergeh(8, t1, t0);
    s0 = b.esimd_mergel(8, t1, t0);
}

inline void p2s(KernelBuilder & b, Value * p[], Value * s[]) {
    Value * bit00004444[2];
    Value * bit22226666[2];
    Value * bit11115555[2];
    Value * bit33337777[2];
    p2s_step(b, p[7], p[3], b.simd_himask(8), 4, bit00004444[1], bit00004444[0]);
    p2s_step(b, p[6], p[2], b.simd_himask(8), 4, bit11115555[1], bit11115555[0]);
    p2s_step(b, p[5], p[1], b.simd_himask(8), 4, bit22226666[1], bit22226666[0]);
    p2s_step(b, p[4], p[0], b.simd_himask(8), 4, bit33337777[1], bit33337777[0]);
    Value * bit00224466[4];
    Value * bit11335577[4];
    for (unsigned j = 0; j<2; j++) {
        p2s_step(b, bit00004444[j], bit22226666[j],b.simd_himask(4), 2, bit00224466[2*j+1], bit00224466[2*j]);
        p2s_step(b, bit11115555[j], bit33337777[j],b.simd_himask(4), 2, bit11335577[2*j+1], bit11335577[2*j]);
    }
    for (unsigned j = 0; j<4; j++) {
        p2s_step(b, bit00224466[j], bit11335577[j], b.simd_himask(2), 1, s[2*j+1], s[2*j]);
    }
}

void P2SKernel::generateDoBlockMethod(KernelBuilder & b) {
...
void P2SMultipleStreamsKernel::generateDoBlockMethod(KernelBuilder & b) {
    Value * input[8];
    unsigned k = 0;
...
P2S16Kernel::P2S16Kernel(KernelBuilder & b, StreamSet *u16bits, StreamSet * u16stream, cc::ByteNumbering numbering)
...
P2S16Kernel::P2S16Kernel(KernelBuilder & b, StreamSets & inputSets, StreamSet * u16stream, cc::ByteNumbering numbering)
...
void P2S16Kernel::generateDoBlockMethod(KernelBuilder & b) {
    Value * lo_input[8];
    Value * hi_input[8];
    unsigned k = 0;
    for (unsigned i = 0; i < getNumOfStreamInputs(); ++i) {
...
P2SKernel::P2SKernel(KernelBuilder & b, StreamSet * basisBits, StreamSet * byteStream)
...
P2SMultipleStreamsKernel::P2SMultipleStreamsKernel(KernelBuilder & b,
...
P2S16KernelWithCompressedOutput::P2S16KernelWithCompressedOutput(KernelBuilder & b,
...
void P2S21Kernel::generateDoBlockMethod(KernelBuilder & b) {
...
P2S21Kernel::P2S21Kernel(KernelBuilder & b, StreamSet *u21bits, StreamSet *u32stream, cc::ByteNumbering numbering)
...
P2S16KernelWithCompressedOutputOld::P2S16KernelWithCompressedOutputOld(KernelBuilder & b, StreamSet * basisBits, StreamSet * delCounts, StreamSet * byteStream)
